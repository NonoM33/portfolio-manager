...
export async function handleCommission(investorId, action, amount) {
  ...
  try {
    ...
    if (action === 'withdraw') {
      ...
      // Reinvest: increase investor capital and initial capital
      await pool.query(
      ...
      // IMPORTANT: Reset entry_ratio to current ratio so commission resets to 0
      // This prevents the investor from claiming the same commission again
      const newInitialCapital = data.initialCapital + commissionAmount
      const newEntryRatio = data.totalCapital / newInitialCapital
      await pool.query(
        'UPDATE investors SET entry_ratio = $1 WHERE id = $2',
        [newEntryRatio, investorId]
      )
